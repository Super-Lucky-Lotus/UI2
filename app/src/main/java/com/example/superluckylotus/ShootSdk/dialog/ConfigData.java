...
public class ConfigData implements Parcelable {
    ...
    //锐动素材服务器地址
    //锐动素材服务器地址
    public static final String BASE_URL_ZH = "http://d.56show.com/filemanage2/public/filemanage/file/";//中文
    public static final String BASE_URL_EN = "http://d.56show.com/filemanage2/public/file_en/file/";//英文
    //素材资源服务器地址  （文字、贴纸、滤镜、转场、特效、AE模板、背景音乐 、mv共用此url）
    public static String APP_DATA = BASE_URL_ZH + "appData";
    //资源分类（因部分功能需要分类，如：AE模板 、特效、云音乐-分类、音效-分类）
    public static String TYPE_URL = BASE_URL_ZH + "typeData";
    //音效、云音乐  （支持分页加载）
    public static String SOUND_URL = BASE_URL_ZH + "cloudMusicData";

    public static void setLanguage(boolean zh) {
        if (zh) {
            APP_DATA = BASE_URL_ZH + "appData";
            TYPE_URL = BASE_URL_ZH + "typeData";
            SOUND_URL = BASE_URL_ZH + "cloudMusicData";
        } else {
            APP_DATA = BASE_URL_EN + "appData";
            TYPE_URL = BASE_URL_EN + "typeData";
            SOUND_URL = BASE_URL_EN + "cloudMusicData";
        }
    }


    @Deprecated   //兼容旧版sdk  （如使用新的素材管理系统，可忽略此部分的定义）
    private static final String DIANBOOK_BASE_URL = "http://dianbook.17rd.com/api/shortvideo/";
    @Deprecated
    public static final String WEB_MV_URL = DIANBOOK_BASE_URL + "getmvprop3";
    @Deprecated
    public static final String MUSIC_URL = DIANBOOK_BASE_URL + "getbgmusic";
    @Deprecated     // 云音乐
    public static final String CLOUDMUSIC_URL = DIANBOOK_BASE_URL + "getcloudmusic";


    //自定义的服务器资源地址
    public String customApi = APP_DATA;
    ...
    public boolean enableMusicMany = true;
    ...
    public boolean enableCover = true;
    public boolean enableGraffiti = true;
    ...
    public int cameraMinTime = 1;
    public int cameraMaxTime = 15;
    public int cameraMVMaxTime = 1;
    public int cameraMVMinTime = 15;
    ...
    public int compressFrameRate = 25;
    ...
    public void writeToParcel(Parcel dest, int flags) {
        dest.writeByte(this.enableVolume ? (byte) 1 : (byte) 0);
        dest.writeByte(this.enableSound ? (byte) 1 : (byte) 0);
        dest.writeByte(this.enableMusicMany ? (byte) 1 : (byte) 0);
        ...
        dest.writeByte(this.enableEffects ? (byte) 1 : (byte) 0);
        ...
        dest.writeByte(this.enableDewatermark ? (byte) 1 : (byte) 0);
        dest.writeByte(this.enableCollage ? (byte) 1 : (byte) 0);
        dest.writeInt(this.compressFrameRate);
        dest.writeByte(this.enableEditSort ? (byte) 1 : (byte) 0);
        dest.writeByte(this.enableCover ? (byte) 1 : (byte) 0);
        dest.writeByte(this.enableGraffiti ? (byte) 1 : (byte) 0);
    }

    protected ConfigData readFromParcel(Parcel in) {
        //音量
        this.enableVolume = in.readByte() != 0;
        //音效、多段配乐
        this.enableSound = in.readByte() != 0;
        this.enableMusicMany = in.readByte() != 0;
        ...
        this.enableEffects = in.readByte() != 0;
        ...
        this.enableSoundEffect = in.readByte() == 1;
        this.enableDewatermark = in.readByte() == 1;
        this.enableCollage = in.readByte() == 1;
        this.compressFrameRate = in.readInt();
        this.enableEditSort = in.readByte() == 1;
        this.enableCover = in.readByte() == 1;
        this.enableGraffiti = in.readByte() == 1;
